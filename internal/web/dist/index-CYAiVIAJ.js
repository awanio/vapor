  ...
  async loadTranslations(e) {
    if (!this.translations.has(e))
      try {
        ...
        this.translations.set(e, n), console.log(`Loaded translations for locale ${e}`);
      } catch (s) {
        console.error(`Failed to load translations for locale ${e}:`, s);
        try {
          const o = `/locales/${e}.json`, t = await fetch(o);
          if (t.ok) {
            const n = await t.json();
            this.translations.set(e, n), console.log(`Loaded translations for locale ${e} from fallback path`);
          }
        } catch (o) {
          console.error(`Fallback also failed for locale ${e}:`, o);
        }
      }
  }
  async setLocale(e) {
  ...
  t(e, s) {
    ...
    for (const r of t)
      if (typeof n == "object" && r in n)
        ...
        return e;
    return typeof n != "string" ? e : s ? n.replace(/\{\{(\w+)\}\}/g, (r, h) => h in s ? String(s[h]) : r) : n;
  }
  onChange(e) {
...
class i {
  ...
  async login(e, s) {
    try {
      const o = await fetch(p("/auth/login"), {
        method: "POST",
        headers: {
...
w.isAuthenticated();
u.init().then(() => {
  console.log("i18n initialized, loading app..."), import("./app-root-Du8VA-jj.js"), console.log("Vapor Web UI initialized");
}).catch((a) => {
