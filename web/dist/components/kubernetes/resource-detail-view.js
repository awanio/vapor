...
let ResourceDetailView = class ResourceDetailView extends LitElement {
    ...
    renderMetadata() {
      ...
      ${hasLabels ? html `
        <div class="detail-section">
          <h3>Labels</h3>
          ${Object.entries(metadata.labels).map(([key, value]) => this.renderDetailItem(key, value))}
        </div>
      ` : ''}
      ...
      ${hasAnnotations ? html `
        <div class="detail-section">
          <h3>Annotations</h3>
          ${Object.entries(metadata.annotations).map(([key, value]) => this.renderDetailItem(key, value))}
        </div>
      ` : ''}
    `;
    }
    ...
    renderStatus() {
      ...
      ${conditions && conditions.length > 0 ? html `
        <div class="detail-section">
          <h3>Conditions</h3>
          ${conditions.map((condition) => this.renderConditionDetails(condition))}
        </div>
      ` : ''}
    `;
    }
    ...
    renderObjectAsDetailItems(obj, isStatus = false) {
        const results = [];
      ...
      <div class="pod-details-content">
        <div class="detail-sections">
          ...
          ${spec.containers && spec.containers.length > 0 ? html `
            <div class="detail-section">
              <h3>Containers</h3>
              ${spec.containers.map((container, index) => this.renderContainerDetails(container, index + 1, status.containerStatuses))}
            </div>
          ` : ''}
          ...
          ${status.conditions && status.conditions.length > 0 ? html `
            <div class="detail-section">
              <h3>Conditions</h3>
              ${status.conditions.map((condition) => this.renderConditionDetails(condition))}
            </div>
          ` : ''}
    ...
    renderContainerDetails(container, index, containerStatuses = []) {
