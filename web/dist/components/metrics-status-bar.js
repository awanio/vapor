...
import { $cpuUsage, $memoryUsage, $metricsConnected, $metricsAlerts, formatBytes, $availableMemory, $loadAverage, } from '../stores/shared/metrics';
let MetricsStatusBar = class MetricsStatusBar extends StoreMixin(LitElement) {
    constructor() {
        super(...arguments);
        this.cpuUsage = this.subscribeToStore($cpuUsage);
        this.memoryUsage = this.subscribeToStore($memoryUsage);
        this.availableMemory = this.subscribeToStore($availableMemory);
        this.loadAverage = this.subscribeToStore($loadAverage);
        this.metricsConnected = this.subscribeToStore($metricsConnected);
        this.metricsAlerts = this.subscribeToStore($metricsAlerts);
    }
    getValueClass(value, _type) {
        if (value > 90)
            return 'critical';
        if (value > 75)
            return 'high';
...
MetricsStatusBar.styles = css `
  ...
  `;
MetricsStatusBar = __decorate([
    customElement('metrics-status-bar')
], MetricsStatusBar);
export { MetricsStatusBar };
export default MetricsStatusBar;
//# sourceMappingURL=metrics-status-bar.js.map