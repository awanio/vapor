...
import type { DockerNetwork, DockerNetworksResponse } from '../types/api';

export class DockerNetworksTab extends LitElement {
  @state() private networks: DockerNetwork[] = [];
  @state() private filteredNetworks: DockerNetwork[] = [];
              ...
              <tr>
                ...
                <td>
                  <div class="action-menu">
                    <button class="action-dots" @click="${(e: Event) => this.toggleActionMenu(e, `network-${network.id}`)}">⋮</button>
  ...
  async deleteNetwork(name: string) {
    if (this.isSystemNetwork({ name } as DockerNetwork)) {
    ...
    try {
      ...
      // await api.delete(`/docker/networks/${name}`);
      // this.fetchNetworks();
    } catch (error) {
