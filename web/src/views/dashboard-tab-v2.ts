/**
 * Dashboard Tab V2 - Uses centralized metrics store
 * Demonstrates the benefits of state management
 */
...
/**
 * Enhanced Dashboard Tab using the metrics store
 * Benefits:
 * 1. Data persists across navigation
 * 2. Historical data is preserved (up to 120 points vs 30)
 * 3. Computed values like trends and averages
 * 4. Alerts based on thresholds
 * 5. Shared data with other components
 * 6. Cleaner component code (separation of concerns)
 */
@customElement('dashboard-tab-v2')
export class DashboardTabV2 extends StoreMixin(I18nLitElement) {
  ...
  private cpuInfo = this.subscribeToStore($cpuInfo);
  private memoryInfo = this.subscribeToStore($memoryInfo);
  private currentCpu = this.subscribeToStore($currentCpu);
  private currentMemory = this.subscribeToStore($currentMemory);
  private cpuHistory = this.subscribeToStore($cpuHistory);
  private memoryHistory = this.subscribeToStore($memoryHistory);
  private metricsConnected = this.subscribeToStore($metricsConnected);
  private metricsError = this.subscribeToStore($metricsError);
  private cpuUsage = this.subscribeToStore($cpuUsage);
  private memoryUsage = this.subscribeToStore($memoryUsage);
  private cpuTrend = this.subscribeToStore($cpuTrend);
  ...
  override firstUpdated() {
    this.initCharts();
    // Update charts with existing history
  ...
  private initCharts() {
    ...
    // Initialize Memory chart with history
  ...
  private renderTrendIndicator(trend: 'increasing' | 'decreasing' | 'stable') {
    const icons = {
      increasing: '↑',
      decreasing: '↓',
      stable: '→',
