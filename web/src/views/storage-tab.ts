...
export class StorageTab extends I18nLitElement {
  ...
  @property({ type: Array }) physicalVolumes: PhysicalVolume[] = [];
  @property({ type: Array }) raidDevices: RAIDDevice[] = [];
  @property({ type: Array }) availableRaidDisks: RAIDDisk[] = [];
  @property({ type: Array }) iscsiTargets: ISCSITarget[] = [];
  ...
  async fetchBTRFS() {
    // For demo purposes, we'll skip this as it requires a mount point
  ...
  // Action methods
  async mountPartition(device: string, mountPoint: string) {
    try {
  ...
  async unmountPartition(mountPoint: string) {
    try {
  ...
  async destroyRAID(device: string) {
    if (!confirm(t('storage.raid.confirmDestroy'))) return;
  ...
  async logoutISCSI(target: string) {
    try {
  ...
  async loginISCSI(target: ISCSITarget) {
    // In a real implementation, this would show a dialog for credentials
    try {
