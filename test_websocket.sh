...
echo '{"type":"auth","payload":{"token":"'$TOKEN'"}}' | websocat -n ws://localhost:8080/ws/terminal &
TERMINAL_PID=$!
...
echo '{"type":"subscribe","payload":{"cols":80,"rows":24,"shell":"/bin/bash"}}' | websocat -n ws://localhost:8080/ws/terminal &

# Wait a bit to see if we get data
sleep 2

# Send input
echo '{"type":"input","data":"ls\n"}' | websocat ws://localhost:8080/ws/terminal >/dev/null 2>&1 &
